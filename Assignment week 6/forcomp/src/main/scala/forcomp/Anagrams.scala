...
object Anagrams {
    ...
    occurrences.foldRight(List[Occurrences](List())) {
      case ((char, num), list) =>
        list ++ (for {
          combination <- list
          charNum <- 1 to num
        } yield (char, charNum) :: combination)
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x) {
      (tot, subY) => {
        val elem = tot.find(subY._1 == _._1).get
        tot.updated(tot.indexOf(elem), (elem._1, elem._2 - subY._2))
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def doSentenceAnagram(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty)
        ...
        for {
          ...
          anagrams <- doSentenceAnagram(subtract(occurrences, comb))
        } yield word :: anagrams
      }
    }
